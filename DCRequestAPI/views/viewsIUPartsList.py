...
class IUPartsListView():
	...
	def IUPartsListJSON(self):
		...
		for doc in docs:
			...
			for colkey in self.search_params['result_table_columns']:
	...
	def IUPartsListHTML(self):
		...
		pagecontent = {
			...
			'bucketdefs': filter_names,
			'hierarchy_filter_names': hierarchy_filter_names,
			# the fields for the user interface
			'default_sourcefields': self.fieldconfig.result_fields,
			'selected_sourcefields': self.search_params['result_table_columns'],
			'available_filter_fields': self.fieldconfig.available_filter_fields,
